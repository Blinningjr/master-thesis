...
In the case of the \emph{Rust} compiler \gls{rustc} it is the \gls{LLVM} library that mostly handles the debug information generation.
\gls{LLVM} is a very lager project that many people are working on and thus improving on \gls{LLVM} is out of scope for this thesis, the same goes for improving \gls{rustc}.
...
The compiler backend \gls{LLVM} that \gls{rustc} uses supports two debugging file formats that holds all the debug information.
...
The \gls{DWARF} format is very complex and is very well explain in there documentation.
