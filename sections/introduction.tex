...
The debug information stored in the \gls{DWARF} file can be used to locate the values of variables.
That in most cases are stored on the call stack in memory, section \ref{sec:callstack} explains what a call stack is.
...
The main problem with debugging optimized code is variables are sometimes located in registers.
...
That mostly have the same life span as the stack frame.
...
One of the larger project focusing on debugging is called \emph{gimli-rs}, one of the main goals of the projects is to make a \emph{Rust} library for reading the debugging format \gls{DWARF}.
...
Also, a debugger is very good tool for showing the state of these peripheral, which makes the development and testing much easier.
Another reason is that hardware bugs are much easier to find if it is possible to inspect the state of a device easily.


% Improving the state of debugging.
Another motivation for creating a debugger for embedded systems is that the two supported debuggers for \emph{Rust} by the \emph{Rust} team are \emph{LLDB} (part of the \emph{LLVM} project) and \emph{GDB}, which at the time of writhing this both requires another program to interact with the \gls{mcu}.
...
Thus, one of the key motivations is to write a debugger in \emph{Rust}, which will also lead to the debugger having all the benefit of memory safety that \emph{Rust} provides, and excellent ecosystem that Rust provides.
...
This will be achieved by creating a new debugger and debugging library that aims to solve the three following problems:
...
The messages that they give are very vague and can make it impossible to understand what is happening the program.

\item Retrieving debugging information form the \gls{DWARF} format is difficult to do. 
This makes creating tools that require debug information difficult and time-consuming to implement when it does not need to be.

\end{itemize}

The goal for this thesis is to create a new debugger and library that fix the problems mentioned above.
...
Thus, it is decided to limit the number of features to the ones that are most important, to keep the scope of this thesis reasonable.
...
Thus, the debugger is only required to work with the target system that is available, which is a \emph{Nucleo-64, STM32F401RET6 64 pin}.
