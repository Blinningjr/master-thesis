...
When a function is finished the return address is written to the \acr{pc} register, this makes the program jump to the return address.
...
The \gls{DWARF} format is divided into sections that all contain unique information, with some few exceptions.
...
The compiler, when compiling a source program, will mostly generate one compilation unit for each project/library.
