...
The documentation for \emph{rust-debug} is a lot less text then the specification for the \gls{DWARF} format.
...
But, when the difference is so large it is hard to deny that the completely has reduced. 
...
But it turn out that \emph{GDB} was right in that they were completely optimized out.


Keeping these expectation in mind, the result in section \ref{sec:debuggercomparison} is disappointing.
...
But, the problems show that \emph{erdb} has some small advantages.
