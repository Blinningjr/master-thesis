...
When debugging the code with optimization $2$, \emph{LLDB} showed no variables at all.
And, \emph{GDB} showed wrong values on both a \emph{i64} and a \emph{u64}.
\emph{GDB} read the values a $32$ bit integers instead of $64$ bit.
Note that \emph{GDB} is able to evaluated both of the values correctly when the code is not optimized.
...
% TODO: Mention that there has not been much improvement.
% TODO: Mention the result of older version.
