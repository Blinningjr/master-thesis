...
\subsection{Accessing Memory And Registers} % TODO
...
This has the benefit that any existing solutions can be used, like \emph{probe-rs}. % todo
...
It also contains the register number of some special registers, like the \gls{pc} register.
Thus, it is up to the user that the correct register numbers and values are stored on the struct.  % TODO reference embedded-rust-debug code as an example
...
This trait is very simple and only has one function, which is for reading \emph{x} amount of bytes at a address given.
% TODO reference embedded-rust-debug code as an example 
