...
Some functionalities in the \gls{DWARF} format requires access to register and memory values.
...
But it also has one downside, \emph{rust-debug} is a little harder to use because of extra work the user has to do.
...
And the other method called \emph{handle\_request} takes a \emph{DebugRequest} as an argument, and calls a function based on the argument.
