...
But it also keeps track of the debug targets state.
The struct only has two methods for accomplishing this, one for them is for polling the state of the debug target.
And the other method called \emph{handle_request} takes a \emph{DebugRequest} as a argument, and calls a function based on the argument.
The method also returns a \emph{DebugResponse} with the result.


The general flow when the \emph{handle_request} method is called is as follows.
First the \emph{DebugRequest} enum is matched and a check is done to see that all the required information is present.
Then, the debugger module will preform some task to related to the given argument.
Which often involve using the \emph{probe-rs} library to access the debug target, and/or the \emph{rust-debug} for retrieving debug information.
...
In the time between the polls other tasks can be handled, such as parsing the input from the \arc{cli} or doing a stack trace.
This is possible because there is a asynchronously function which sleeps for $100$ millisecond, afterwards it polls the debug target state.
...
A state change in the debug target is called a event in \gls{erdb}.
...
This is to ensures that the correct information is returned.
...
Because, the value does not need to be evaluate, instead it can be taken from temporarily stored information.



\section{VSCode Extension} % TODO
