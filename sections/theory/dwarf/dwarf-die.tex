...
The \glspl{die} are found in a \gls{tree} structured referred to a as a \gls{die} \gls{tree}.
Each \gls{die} \gls{tree} will most often represents a whole compile unit or a type from the source code.
...
The \glspl{die} representing types are often referred to a type \glspl{die}.
...
\subsubsubsection{Example of a \gls{die}}
Lets look at an example of a \gls{die} that describes a variable, the example can be seen in figure \ref{fig:dwarfdie} which is a screen shoot of the output from the program \emph{objdump} run on a \gls{elf} file.
