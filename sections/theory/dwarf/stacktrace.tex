...
The reason for stopping could be that a breakpoint was hit, it was interrupted by an event or it could be a location were it made a call to the next subroutine.
...
This works for all activations except for the top activation, were the current \gls{pc} value is the stopped code location.
...
For further details we refer the reader to section \ref{sec:evalcfa}.
...
That is done by finding the closes address that is less than the search one.
